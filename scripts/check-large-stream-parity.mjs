...
const source = `<style>
  main { width: 440px; padding: 12px; border: 1px solid #334155; background: #f8fafc; }
  p { margin: 0 0 5px 0; font-size: 10px; }
</style><main>${
  Array.from({ length: 500 }, (_, index) => `<p>Bounded continuation row ${index + 1}</p>`).join("")
}</main>`;

const root = mkdtempSync(join(tmpdir(), "boxpdf-html-large-stream-"));
try {
  ...
  if (bufferedPages.length !== streamedPages.length) {
    throw new Error(`large wrapper page count differs (${bufferedPages.length} buffered, ${streamedPages.length} streamed)`);
  }
  for (let index = 0; index < bufferedPages.length; index += 1) {
    if (!readFileSync(bufferedPages[index]).equals(readFileSync(streamedPages[index]))) {
      throw new Error(`large wrapper page ${index + 1} differs`);
    }
  }
  console.log(`large streamed wrapper: ${bufferedPages.length} page(s) match`);
} finally {
